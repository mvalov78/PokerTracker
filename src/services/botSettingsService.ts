...
export class BotSettingsService {
  ...
  static async getBotSettings(): Promise<BotSettings | null> {
    try {
      ...
      data?.forEach(setting => {
        ...
        // Преобразуем типы
        if (key === 'polling_enabled' || key === 'webhook_enabled' || key === 'auto_restart') {
          value = value === 'true'
   ...
   * Обновить настройку
   */
  static async updateSetting(key: keyof BotSettings, value: string | boolean | number): Promise<boolean> {
    try {
      const supabase = createAdminClient()
      if (!supabase) {
        console.warn('Admin client not available')
  ...
  static async getBotMode(): Promise<'polling' | 'webhook' | null> {
    const mode = await this.getSetting('bot_mode')
   ...
   */
  static async updateBotStatus(status: 'active' | 'inactive' | 'error'): Promise<boolean> {
    return await this.updateSetting('bot_status', status)
  }
  ...
  static async incrementErrorCount(): Promise<void> {
    try {
      const currentCount = await this.getSetting('error_count')
      const newCount = (parseInt(currentCount || '0') + 1).toString()
      await this.updateSetting('error_count', newCount)
      await this.updateSetting('last_update_time', new Date().toISOString())
    } catch (error) {
  ...
  static async resetErrorCount(): Promise<void> {
    await this.updateSetting('error_count', '0')
  }
  ...
  static async getWebhookUrl(): Promise<string | null> {
    return await this.getSetting('webhook_url')
  }
