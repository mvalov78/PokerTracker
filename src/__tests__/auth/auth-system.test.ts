...
import { createClientComponentClient, createAdminClient, getProfile, createProfile, isAdmin, getUserOrCreate } from '@/lib/supabase'
...
describe('Authentication System Tests', () => {
  ...
  describe('Error Handling', () => {
    test('should handle profile creation errors', async () => {
      ...
      await expect(createProfile('test-user-id', { username: 'test' }))
        .rejects.toThrow('Database error')
    })

    test('should handle getUserOrCreate errors', async () => {
      ;(getUserOrCreate as jest.Mock).mockRejectedValue(new Error('Failed to create user'))

      await expect(getUserOrCreate(123456789))
