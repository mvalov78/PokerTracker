...
describe("lib/utils", () => {
  ...
  describe("isImageFile", () => {
    it("should identify image files", () => {
      expect(
        isImageFile(new File([], "test.jpg", { type: "image/jpeg" })),
      ).toBe(true);
      expect(isImageFile(new File([], "test.png", { type: "image/png" }))).toBe(
        true,
      );
    });
    ...
    it("should reject non-image files", () => {
      expect(
        isImageFile(new File([], "test.txt", { type: "text/plain" })),
      ).toBe(false);
      expect(
        isImageFile(new File([], "test.pdf", { type: "application/pdf" })),
      ).toBe(false);
    });
  });
