...
describe('Tournament Flow Integration Tests', () => {
  ...
  it('should handle form cancellation', async () => {
    ...
    // Заполняем частично
    await user.type(screen.getByTestId('tournament-name'), 'Partial Tournament')
  ...
  it('should handle tournament deletion', async () => {
    ...
    // Создаем турнир
    await user.click(screen.getByTestId('toggle-form'))
    await user.type(screen.getByTestId('tournament-name'), 'Tournament to Delete')
    ...
    await waitFor(() => {
      expect(screen.queryByText('Tournament to Delete')).not.toBeInTheDocument()
    })
    expect(screen.getByText('Нет турниров')).toBeInTheDocument()
  })
})
