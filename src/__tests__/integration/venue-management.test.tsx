...
const mockUserSettingsService = UserSettingsService as jest.Mocked<typeof UserSettingsService>
    ...
    <div data-testid="venue-manager">
      ...
      <div data-testid="current-venue-section">
        ...
        {isLoading ? (
          ...
          <p data-testid="current-venue">
            {currentVenue || 'Не установлена'}
          </p>
        )}
      </div>
  ...
  return (
    <div data-testid="tournament-with-venue">
      ...
      <input
        ...
        onChange={(e) => setTournamentData(prev => ({ ...prev, venue: e.target.value }))}
      ...
      <button
        onClick={() => {
          ...
          const finalVenue = currentVenue || tournamentData.venue || 'Не указана'
...
describe('Venue Management Integration Tests', () => {
  ...
  it('should display current venue when loaded', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('current-venue')).toHaveTextContent('Casino Royale')
    })

    expect(mockUserSettingsService.getCurrentVenue).toHaveBeenCalledWith('test-user')
  })
  ...
  it('should display "Не установлена" when no venue is set', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('current-venue')).toHaveTextContent('Не установлена')
    })
  })
  ...
  it('should update venue successfully', async () => {
    ...
    // Wait for update
    await waitFor(() => {
      expect(screen.getByTestId('current-venue')).toHaveTextContent('New Casino')
    })

    expect(mockUserSettingsService.updateCurrentVenue).toHaveBeenCalledWith('test-user', 'New Casino')
  })
  ...
  it('should handle venue loading errors gracefully', async () => {
    mockUserSettingsService.getCurrentVenue.mockRejectedValue(new Error('Database error'))
  ...
  it('should handle venue update errors gracefully', async () => {
    ...
    mockUserSettingsService.updateCurrentVenue.mockRejectedValue(new Error('Update failed'))
    ...
    await waitFor(() => {
      expect(screen.getByTestId('update-venue-button')).not.toHaveTextContent('Обновление...')
    })
  ...
  it('should refresh venue when refresh button is clicked', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('current-venue')).toHaveTextContent('Initial Casino')
    })
    ...
    await waitFor(() => {
      expect(screen.getByTestId('current-venue')).toHaveTextContent('Updated Casino')
    })
  ...
  describe('Tournament Creation with Venue Priority', () => {
    it('should use current venue for new tournament', async () => {
      mockUserSettingsService.getCurrentVenue.mockResolvedValue('Default Casino')
      ...
      await waitFor(() => {
        expect(screen.getByText('Текущая площадка: Default Casino')).toBeInTheDocument()
        expect(screen.getByText('Площадка турнира: Default Casino')).toBeInTheDocument()
      })

      // Tournament venue input should be pre-filled
      expect(screen.getByTestId('tournament-venue-input')).toHaveValue('Default Casino')
    })

    it('should prioritize current venue over manual input', async () => {
      mockUserSettingsService.getCurrentVenue.mockResolvedValue('Priority Casino')
      ...
      await waitFor(() => {
        expect(screen.getByText('Текущая площадка: Priority Casino')).toBeInTheDocument()
      })
      ...
      await waitFor(() => {
        expect(screen.getByText('Площадка турнира: Priority Casino')).toBeInTheDocument()
      })
    })
    ...
    it('should use manual venue when no current venue is set', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText('Текущая площадка: Не установлена')).toBeInTheDocument()
      })

      // Set manual venue
      await user.type(screen.getByTestId('tournament-venue-input'), 'Manual Casino')

      // Create tournament
      await user.click(screen.getByTestId('create-tournament-button'))

      await waitFor(() => {
        expect(screen.getByText('Площадка турнира: Manual Casino')).toBeInTheDocument()
      })
    })
  })
})
