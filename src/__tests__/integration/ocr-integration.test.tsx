...
describe('OCR Integration Tests', () => {
  ...
  it('should handle complete OCR workflow', async () => {
    ...
    const file = new File(['test image content'], 'ticket.jpg', { type: 'image/jpeg' })
  ...
  it('should handle OCR failure gracefully', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByText('🎯 Извлеченные данные')).not.toBeInTheDocument()
    }, { timeout: 5000 })
  ...
  it('should handle low confidence results', async () => {
    ...
    const file = new File(['blurry image'], 'blurry-ticket.jpg', { type: 'image/jpeg' })
    const fileInput = document.querySelector('input[type="file"]') as HTMLInputElement
  ...
  it('should handle drag and drop upload', async () => {
    ...
    const file = new File(['test image'], 'dropped-ticket.jpg', { type: 'image/jpeg' })
    const dropZone = screen.getByText('Загрузите фото билета турнира').closest('div')
  ...
  it('should allow file removal and reset', async () => {
    ...
    expect(screen.queryByText('Предпросмотр билета')).not.toBeInTheDocument()
    expect(screen.getByText('Загрузите фото билета турнира')).toBeInTheDocument()
  })
  ...
  it('should handle multiple confidence levels correctly', async () => {
    ...
    for (const { confidence, expectedClass } of confidenceLevels) {
      ...
      const file = new File(['test'], `ticket-${confidence}.jpg`, { type: 'image/jpeg' })
      const fileInput = document.querySelector('input[type="file"]') as HTMLInputElement
      ...
      await waitFor(() => {
        const confidenceBadge = screen.getByText(`Точность: ${Math.round(confidence * 100)}%`)
