  ...
  async handleDocumentAsPhoto(ctx: BotContext) {
    try {
      ...
      if (!document.mime_type?.startsWith('image/')) {
        await ctx.reply('❌ Файл не является изображением. Пожалуйста, отправьте фото билета.')
        return
      }
      ...
      console.log('🔍 [handleDocumentAsPhoto] Получаем текущую площадку пользователя:', telegramId)
      const currentVenueDoc = await UserSettingsService.getCurrentVenue(telegramId)
      console.log('🏨 [handleDocumentAsPhoto] Текущая площадка пользователя:', currentVenueDoc)
      console.log('🏨 [handleDocumentAsPhoto] Площадка из OCR:', data.venue)
      const finalVenueDoc = currentVenueDoc || data.venue || 'Не указана'
      console.log('🏨 [handleDocumentAsPhoto] Финальная площадка для отображения:', finalVenueDoc)

      // Сохраняем данные в сессии (с подстановкой финальной площадки)
      ctx.session.ocrData = { ...data, venue: finalVenueDoc }
      console.log('💾 Данные OCR из документа сохранены в сессии (с финальной площадкой):', JSON.stringify(ctx.session.ocrData, null, 2))
...
Все верно? Нажмите ✅ для подтверждения или ❌ для отмены.
      ...
      if (error instanceof Error) {
        ...
        } else if (error.message.includes('OCR') || error.message.includes('processing')) {
      ...
      errorMessage += '• Убедиться, что изображение четкое и билет полностью виден\n'
  ...
  async handlePhoto(ctx: BotContext) {
    try {
      ...
      const currentVenue = await UserSettingsService.getCurrentVenue(telegramId)
      ...
      console.log('💾 Данные OCR сохранены в сессии:', JSON.stringify(data, null, 2))
      ...
      console.log('🔍 [handlePhoto] currentVenue !== data.venue:', currentVenue !== data.venue)
      
      if (currentVenue && currentVenue !== data.venue) {
        venueText += ` *(установлена как текущая)*`
        console.log('🔍 [handlePhoto] Добавлена пометка, venueText:', venueText)
      } else {
  ...
  async confirmTournament(ctx: BotContext) {
    try {
      ...
      if (!data) {
        ...
        await ctx.reply('❌ Данные OCR не найдены. Попробуйте отправить фото еще раз.')
        return
      }
      ...
      console.log('🔍 [confirmTournament] Получаем UUID пользователя по Telegram ID:', telegramId)
      ...
      const currentVenue = await UserSettingsService.getCurrentVenue(telegramId)
      console.log('🏨 [confirmTournament] Текущая площадка пользователя:', currentVenue)
      console.log('🏨 [confirmTournament] Площадка из OCR:', data.venue)
      let venue = currentVenue || data.venue || 'Не указано'
...
Для отмены введите \`cancel\`
   ...
   */
  private async savePhotoToHistory(tournamentId: string, fileId: string, ocrData: any) {
