...
Добро пожаловать в PokerTracker Pro! 🚀
      ...
      await ctx.reply('❌ Произошла ошибка при связывании аккаунтов. Попробуйте позже.')
    }
  }
...
• Используйте уведомления для напоминаний
    ...
    try {
      ...
      try {
        await ctx.reply('🤖 Справка по командам PokerTracker Pro Bot\n\n/start - Главное меню\n/help - Эта справка\n/register - Регистрация турнира\n/result - Добавить результат\n/stats - Статистика\n/tournaments - Список турниров\n/venue - Текущая площадка\n/setvenue <название> - Установить площадку\n/settings - Настройки')
        console.log('✅ Упрощенная справка отправлена')
      } catch (simpleError) {
        console.error('❌ Ошибка при отправке упрощенной справки:', simpleError)
      }
    }
  }
...
После турнира используйте /result для добавления результата.
  ...
  async addResult(ctx: BotContext) {
    try {
      ...
      const tournamentsWithoutResults = tournaments.filter((t: any) => !t.result)
      ...
      let message = '🏆 **Выберите турнир для добавления результата:**\n\n'
      
      const keyboard = tournamentsWithoutResults.slice(0, 10).map(tournament => [{
...
Для отмены введите /cancel
  ...
  async handleResultInput(ctx: BotContext, text: string) {
    ...
    try {
      ...
      const roiText = result.roi > 0 ? `+${result.roi.toFixed(1)}%` : `${result.roi.toFixed(1)}%`
...
Результат сохранен в вашем профиле.
      ...
      await ctx.reply('❌ Ошибка при добавлении результата. Попробуйте еще раз.')
    }
  }
  ...
  async getStats(ctx: BotContext) {
    try {
      ...
      const roiText = stats.roi > 0 ? `+${stats.roi.toFixed(1)}%` : `${stats.roi.toFixed(1)}%`
      const profitText = stats.profit > 0 ? `+$${stats.profit}` : `-$${Math.abs(stats.profit)}`
...
Выберите, какие уведомления вы хотите получать:
    ...
    const keyboard = [
      [
        { text: '🔔 Напоминания о турнирах', callback_data: 'notification_toggle:reminders' },
      ],
      [
        { text: '📊 Еженедельная статистика', callback_data: 'notification_toggle:weekly_stats' },
      ],
      [
        { text: '🎯 Достижения', callback_data: 'notification_toggle:achievements' },
      ]
    ]
...
Или введите \`done\` для завершения, \`cancel\` для отмены.
    ...
    if (!tournamentData) {
      await ctx.reply('❌ Данные турнира не найдены. Попробуйте загрузить фото еще раз.')
      session.currentAction = undefined
      return
    }
    ...
    switch (field.toLowerCase()) {
      ...
      case 'date':
        ...
        if (dateValue) {
          ...
          await ctx.reply(`✅ Дата изменена на: ${new Date(dateValue).toLocaleDateString('ru-RU')}`)
        } else {
          await ctx.reply('❌ Неверный формат даты. Используйте: ДД.ММ.ГГГГ или ДД/ММ/ГГГГ')
        }
        break
      ...
      case 'buyin':
        ...
        if (buyinValue > 0) {
          ...
          await ctx.reply('❌ Неверное значение бай-ина. Введите положительное число.')
        }
        break
      ...
      default:
        await ctx.reply(`❌ Неизвестное поле: ${field}. Доступные поля: name, date, buyin, venue`)
        break
    }
  ...
  private async finalizeTournamentEdit(ctx: BotContext) {
    ...
    try {
      ...
      } else if (!tournamentData.venue || tournamentData.venue === 'Не указано' || tournamentData.venue === 'Не указана') {
  ...
  private parseDate(dateStr: string): string | null {
    ...
    for (const format of formats) {
      ...
      if (match) {
        ...
        if (format === formats[2]) { // ГГГГ-ММ-ДД
          [, year, month, day] = match
        } else { // ДД.ММ.ГГГГ или ДД/ММ/ГГГГ
...
• \`/setvenue Онлайн PokerStars\`
      ...
      if (venueName.length < 2) {
        await ctx.reply('❌ Название площадки слишком короткое (минимум 2 символа).')
        return
      }

      if (venueName.length > 100) {
        await ctx.reply('❌ Название площадки слишком длинное (максимум 100 символов).')
        return
      }

      // Сохраняем площадку
      const updatedSettings = await UserSettingsService.updateCurrentVenue(userId, venueName)
...
Для просмотра текущей площадки: \`/venue\`
        ...
        await ctx.reply('❌ Ошибка при сохранении площадки. Попробуйте еще раз.')
      }
  ...
  private calculateStats(tournaments: any[]) {
    ...
    tournaments.forEach(tournament => {
      ...
      if (tournament.result) {
        ...
        if (bestPosition === null || tournament.result.position < bestPosition) {
    ...
    const itmRate = totalTournaments > 0 ? (itmCount / totalTournaments) * 100 : 0
