...
Просто отправьте фото билета для автоматической регистрации турнира.
    ...
    errors: {
      generic: '❌ Произошла ошибка. Попробуйте еще раз или обратитесь к администратору.',
      unauthorized: '🚫 У вас нет доступа к этому боту.',
      rateLimit: '⏳ Слишком много запросов. Подождите немного и попробуйте снова.',
      ocrFailed: '❌ Не удалось распознать данные на билете. Попробуйте сделать более четкое фото.'
    }
  }
}
...
export function logBotConfig(config: BotConfig) {
  ...
  console.log(`  - Уведомления включены: ${config.features.notificationsEnabled}`)
  console.log(`  - Аналитика включена: ${config.features.analyticsEnabled}`)
  console.log(`  - Rate limit: ${config.rateLimit.maxRequests} запросов за ${config.rateLimit.windowMs}ms`)
  
  if (config.allowedUsers) {
    console.log(`  - Разрешенные пользователи: ${config.allowedUsers.length} пользователей`)
  } else {
