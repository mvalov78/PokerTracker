...
export function addTournament(tournamentData: Partial<Tournament>, userId: string = 'user-1'): Tournament {
  ...
  if (typeof window !== 'undefined') {
    window.dispatchEvent(new CustomEvent('tournamentAdded', { 
...
export function addTournamentResult(tournamentId: string, resultData: Partial<Tournament['result']>): Tournament | null {
  ...
  const totalCost = tournament.buyin + (resultData.rebuyCount || 0) * tournament.buyin + (resultData.addonCount || 0) * tournament.buyin
  updatedResult.profit = updatedResult.payout - totalCost
  updatedResult.roi = totalCost > 0 ? (updatedResult.profit / totalCost) * 100 : 0
  
  // Добавляем запись в историю изменений
  const changedFields = Object.keys(updatedResult).filter(field => 
...
export function updateTournamentResult(tournamentId: string, resultData: Partial<Tournament['result']>): Tournament | null {
  ...
  const updatedResult = {
    ...
    payout: resultData.payout !== undefined ? resultData.payout : tournament.result.payout,
    notes: resultData.notes !== undefined ? resultData.notes : tournament.result.notes,
    knockouts: resultData.knockouts !== undefined ? resultData.knockouts : tournament.result.knockouts,
    rebuyCount: resultData.rebuyCount !== undefined ? resultData.rebuyCount : tournament.result.rebuyCount,
    addonCount: resultData.addonCount !== undefined ? resultData.addonCount : tournament.result.addonCount,
    timeEliminated: resultData.timeEliminated !== undefined ? resultData.timeEliminated : tournament.result.timeEliminated,
    finalTableReached: resultData.finalTableReached !== undefined ? resultData.finalTableReached : tournament.result.finalTableReached
  }
  
  // Пересчитываем ROI и прибыль
  const totalCost = tournament.buyin + (updatedResult.rebuyCount || 0) * tournament.buyin + (updatedResult.addonCount || 0) * tournament.buyin
  updatedResult.profit = updatedResult.payout - totalCost
  updatedResult.roi = totalCost > 0 ? (updatedResult.profit / totalCost) * 100 : 0
  
  // Добавляем запись в историю изменений
  const changedFields = Object.keys(resultData).filter(field => 
    resultData[field as keyof typeof resultData] !== tournament.result?.[field as keyof Tournament['result']]
  )
...
export function addBankrollTransaction(transactionData: Omit<BankrollTransaction, 'id' | 'createdAt'>): BankrollTransaction {
...
export function getBankrollSummary(userId: string): BankrollSummary {
  ...
  const roi = settings.initialBankroll > 0 ? (netProfit / settings.initialBankroll) * 100 : 0
...
export function getResultHistory(userId: string, tournamentId?: string): ResultChangeHistory[] {
...
export function addResultHistoryEntry(entry: Omit<ResultChangeHistory, 'id' | 'timestamp'>): void {
