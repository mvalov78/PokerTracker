  ...
  const loadBotSettings = async () => {
    try {
      ...
      if (data.success) {
        ...
        setWebhookUrl(data.settings.webhook_url || window.location.origin + '/api/bot/webhook')
      } else {
        setError(data.error)
      }
    } catch (err) {
  ...
  if (isLoading) {
    return (
      <div className="bg-gray-50 border border-gray-200 rounded-lg p-6">
        ...
        <p className="text-center text-gray-600 mt-2">Загрузка настроек бота...</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        <h2 className="text-2xl font-bold text-gray-900 mb-2">🤖 Управление Telegram ботом</h2>
        <p className="text-gray-600">Переключение между polling и webhook режимами</p>
      </div>

      {/* Текущий статус */}
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">📊 Текущий статус</h3>
        
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-gray-50 rounded-lg p-4">
            <div className="text-sm font-medium text-gray-500">Режим работы</div>
          ...
          <div className="bg-gray-50 rounded-lg p-4">
            ...
            <div className={`text-lg font-bold ${
              parseInt(settings?.error_count || '0') > 0 ? 'text-red-600' : 'text-green-600'
            }`}>
      ...
      {isWebhookMode && webhookInfo && (
        <div className="bg-white border border-gray-200 rounded-lg p-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">🔗 Webhook информация</h3>
          
          <div className="space-y-2 text-sm">
            <div><strong>URL:</strong> <code className="bg-gray-100 px-2 py-1 rounded">{webhookInfo.url}</code></div>
            <div><strong>Ожидающие обновления:</strong> {webhookInfo.pending_update_count}</div>
            <div><strong>Максимум соединений:</strong> {webhookInfo.max_connections}</div>
            
            {webhookInfo.last_error_date && (
              <div className="bg-red-50 border border-red-200 rounded p-3 mt-4">
                <div><strong>Последняя ошибка:</strong> {new Date(webhookInfo.last_error_date * 1000).toLocaleString()}</div>
                <div className="text-red-600">{webhookInfo.last_error_message}</div>
              </div>
            )}
          </div>
        </div>
      ...
      {/* Управление режимами */}
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">⚙️ Переключение режимов</h3>
        
        <div className="space-y-6">
          {/* Polling режим */}
          <div className={`border-2 rounded-lg p-4 ${isPollingMode ? 'border-blue-500 bg-blue-50' : 'border-gray-200'}`}>
            <div className="flex items-center justify-between">
              <div>
                <h4 className="text-lg font-medium text-gray-900">🔄 Polling режим</h4>
                <p className="text-gray-600">Бот опрашивает Telegram сервер каждые несколько секунд</p>
                <div className="text-sm text-gray-500 mt-2">
                  <strong>Преимущества:</strong> Простота настройки, работает везде<br/>
                  <strong>Недостатки:</strong> Больше ресурсов, задержка в ответах
                </div>
              </div>
              ...
              >
                {isSwitching ? '⏳ Переключение...' : isPollingMode ? '✅ Активен' : 'Включить Polling'}
              </button>
            </div>
          </div>

          {/* Webhook режим */}
          <div className={`border-2 rounded-lg p-4 ${isWebhookMode ? 'border-green-500 bg-green-50' : 'border-gray-200'}`}>
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  <h4 className="text-lg font-medium text-gray-900">🔗 Webhook режим</h4>
                  <p className="text-gray-600">Telegram отправляет обновления напрямую на ваш сервер</p>
                  <div className="text-sm text-gray-500 mt-2">
                    <strong>Преимущества:</strong> Мгновенные ответы, меньше ресурсов<br/>
                    <strong>Недостатки:</strong> Требует HTTPS, настройка сложнее
                  </div>
                </div>
                ...
                >
                  {isSwitching ? '⏳ Переключение...' : isWebhookMode ? '✅ Активен' : 'Включить Webhook'}
                </button>
              </div>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
        ...
        <div className="space-y-3 text-sm text-blue-800">
          <div>
            <strong>Polling режим:</strong> Рекомендуется для разработки и тестирования. 
            Бот сам опрашивает Telegram сервер на наличие новых сообщений.
          </div>
          ...
          <div>
            <strong>Переключение:</strong> При переключении режимов бот автоматически останавливается 
            и перезапускается в новом режиме.
          </div>
        </div>
      </div>
    </div>
