...
export default function BotManagementPage() {
  ...
  const switchMode = async (mode: 'polling' | 'webhook', webhookUrl?: string) => {
    try {
      setLoading(true)
      const response = await fetch('/api/admin/bot-mode', {
        method: 'POST',
  ...
  const getStatusDisplay = () => {
    ...
    if (status === 'error') {
      return { icon: '⚠️', text: 'Бот в состоянии ошибки', color: 'text-red-600' }
    }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6 mb-8">
          ...
          <div className="flex items-center space-x-3 mb-4">
            ...
            <div>
              <div className={`font-semibold ${statusDisplay.color} dark:${statusDisplay.color}`}>
                {statusDisplay.text}
              </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  <span className="ml-2 font-medium text-gray-900 dark:text-white">
                    {botStatus.currentMode === 'webhook' ? 'Webhook' : 'Polling'}
                  </span>
                </div>
                <div>
                  <span className="text-gray-600 dark:text-gray-400">Статус:</span>
                  <span className="ml-2 font-medium text-gray-900 dark:text-white">
                    {botStatus.botStatus === 'active' ? 'Активен' : botStatus.botStatus === 'inactive' ? 'Остановлен' : 'Ошибка'}
                  </span>
                </div>
                {botStatus.currentMode === 'webhook' && botStatus.webhookInfo && (
                  <>
                    ...
                    {botStatus.webhookInfo.pending_update_count > 0 && (
                      <div className="col-span-2">
                        <span className="text-yellow-600">⚠️ Ожидающих обновлений: {botStatus.webhookInfo.pending_update_count}</span>
                      </div>
                    )}
                    {botStatus.webhookInfo.last_error_message && (
                      <div className="col-span-2">
                        <span className="text-red-600">❌ Последняя ошибка: {botStatus.webhookInfo.last_error_message}</span>
                      </div>
                    )}
                  </>
                )}
          ...
          <div className="flex space-x-2 mt-4">
            ...
            <button
              onClick={() => {
                if (botStatus?.currentMode === 'polling') {
                  const url = prompt('Введите URL для webhook (например: https://your-app.vercel.app/api/telegram/webhook):')
                  if (url) {
            ...
            >
              🔄 Переключить на {botStatus?.currentMode === 'polling' ? 'Webhook' : 'Polling'}
            </button>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6 mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-gray-700 dark:text-gray-300">Регистрация турниров</span>
            </div>
            <div className="flex items-center space-x-2">
              <span className="text-green-500">✅</span>
              <span className="text-gray-700 dark:text-gray-300">Добавление результатов</span>
            </div>
            <div className="flex items-center space-x-2">
              <span className="text-green-500">✅</span>
              <span className="text-gray-700 dark:text-gray-300">OCR распознавание билетов</span>
            </div>
            <div className="flex items-center space-x-2">
              <span className="text-green-500">✅</span>
              <span className="text-gray-700 dark:text-gray-300">Статистика игрока</span>
            </div>
            <div className="flex items-center space-x-2">
              <span className="text-green-500">✅</span>
            ...
            <div className="flex items-center space-x-2">
              <span className="text-green-500">✅</span>
              <span className="text-gray-700 dark:text-gray-300">Интеграция с веб-приложением</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6 mb-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            ...
            >
              <code className="text-blue-600 dark:text-blue-400 font-mono text-sm">/start</code>
              <div className="text-xs text-gray-600 dark:text-gray-400 mt-1">Приветствие</div>
            </button>
            ...
            >
              <code className="text-green-600 dark:text-green-400 font-mono text-sm">/stats</code>
              <div className="text-xs text-gray-600 dark:text-gray-400 mt-1">Статистика</div>
            </button>
            ...
            >
              <code className="text-purple-600 dark:text-purple-400 font-mono text-sm">/help</code>
              <div className="text-xs text-gray-600 dark:text-gray-400 mt-1">Справка</div>
            </button>
            ...
            >
              <code className="text-orange-600 dark:text-orange-400 font-mono text-sm">/tournaments</code>
              <div className="text-xs text-gray-600 dark:text-gray-400 mt-1">Турниры</div>
            </button>
            ...
            >
              <code className="text-red-600 dark:text-red-400 font-mono text-sm">/register</code>
              <div className="text-xs text-gray-600 dark:text-gray-400 mt-1">Регистрация</div>
            </button>
            ...
            >
              <code className="text-gray-600 dark:text-gray-400 font-mono text-sm">/settings</code>
              <div className="text-xs text-gray-600 dark:text-gray-400 mt-1">Настройки</div>
            </button>
          </div>
          ...
          <div className="mt-4 p-3 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg">
            <div className="flex items-start space-x-2">
              ...
              <div className="text-sm text-yellow-800 dark:text-yellow-200">
                <strong>Симуляция команд:</strong> Нажмите на кнопку команды, чтобы протестировать её выполнение. Результат будет отображен в консоли сервера.
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
              <div>
                <code className="text-blue-600 dark:text-blue-400 font-mono">/start</code>
                <span className="ml-2 text-gray-600 dark:text-gray-400">Приветствие и инструкции</span>
              </div>
            </div>
            <div className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
              <div>
                <code className="text-blue-600 dark:text-blue-400 font-mono">/register</code>
                <span className="ml-2 text-gray-600 dark:text-gray-400">Регистрация нового турнира</span>
              </div>
            </div>
            <div className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
              <div>
                <code className="text-blue-600 dark:text-blue-400 font-mono">/result</code>
                <span className="ml-2 text-gray-600 dark:text-gray-400">Добавление результата турнира</span>
              </div>
            </div>
            <div className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
              <div>
                <code className="text-blue-600 dark:text-blue-400 font-mono">/stats</code>
                <span className="ml-2 text-gray-600 dark:text-gray-400">Статистика игрока</span>
              </div>
            </div>
            <div className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
              <div>
                <code className="text-blue-600 dark:text-blue-400 font-mono">/settings</code>
                <span className="ml-2 text-gray-600 dark:text-gray-400">Настройки уведомлений</span>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="flex items-start space-x-3">
            ...
            <div>
              ...
              <div className="mt-2 space-y-3">
                <div>
                  <h4 className="font-medium text-blue-800 dark:text-blue-200 text-sm">🔄 Polling режим (Long Polling)</h4>
                  <p className="text-blue-700 dark:text-blue-300 text-xs mt-1">
                    Бот периодически опрашивает сервер Telegram на наличие новых сообщений.
                  </p>
                  <div className="mt-1 text-xs text-blue-600 dark:text-blue-400">
                    <div>• <strong>Преимущества:</strong> Простая настройка, работает локально</div>
                    <div>• <strong>Недостатки:</strong> Больше нагрузки на сервер, задержки в ответах</div>
                  </div>
                </div>
                
                <div>
                  <h4 className="font-medium text-blue-800 dark:text-blue-200 text-sm">🔗 Webhook режим</h4>
                  <p className="text-blue-700 dark:text-blue-300 text-xs mt-1">
                    Telegram отправляет обновления напрямую на ваш сервер через webhook URL.
                  </p>
                  <div className="mt-1 text-xs text-blue-600 dark:text-blue-400">
                    <div>• <strong>Преимущества:</strong> Мгновенные ответы, меньше нагрузки на сервер</div>
                    <div>• <strong>Недостатки:</strong> Требует публичный HTTPS URL</div>
                    <div>• <strong>Для продакшена:</strong> ✅ Рекомендуется использовать webhook режим</div>
                  </div>
                </div>
              </div>
            </div>
