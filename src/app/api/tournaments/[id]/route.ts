  ...
  try {
    ...
    if (body.result) {
      ...
      if (tournament.result) {
        ...
        await TournamentService.addTournamentResult(
          tournamentId,
          finalResultData,
        );
      }
    ...
    if (Object.keys(tournamentUpdates).length > 0) {
      ...
      if (tournamentUpdates.tournamentType !== undefined)
        updateData.tournamentType = tournamentUpdates.tournamentType;
      ...
      if (tournamentUpdates.prizePool !== undefined)
        updateData.prizePool = tournamentUpdates.prizePool
          ? parseFloat(tournamentUpdates.prizePool)
          : null;
      if (tournamentUpdates.blindLevels !== undefined)
        updateData.blindLevels = tournamentUpdates.blindLevels;
      if (tournamentUpdates.startingStack !== undefined)
        updateData.startingStack = tournamentUpdates.startingStack
          ? parseInt(tournamentUpdates.startingStack)
          : null;
      if (tournamentUpdates.ticketImageUrl !== undefined)
        updateData.ticketImageUrl = tournamentUpdates.ticketImageUrl;
